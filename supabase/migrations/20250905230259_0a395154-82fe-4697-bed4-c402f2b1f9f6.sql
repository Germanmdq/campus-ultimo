...
ON assignments 
FOR UPDATE 
USING ((auth.uid() = user_id) AND (status = 'submitted'::assignment_status));
...
ON courses 
FOR SELECT 
USING ((auth.role() = 'authenticated'::text) AND (published_at IS NOT NULL));
...
ON forum_post_files 
FOR INSERT 
WITH CHECK (EXISTS ( SELECT 1 FROM forum_posts WHERE ((forum_posts.id = forum_post_files.post_id) AND (forum_posts.author_id = auth.uid()))));
...
CREATE POLICY "Admins can update progress approval" 
ON lesson_progress 
FOR UPDATE 
